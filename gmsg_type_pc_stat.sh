...
cmd_get_all_logids="cat "${log_file_names}" | grep -E 'aj/groupmsg/pubconfirm' | grep -E '\"ua\":\"PC\"' | awk -F \"|\" '{ print \$3 }' >> "${all_logids_name};
...
cmd_get_all_params="cat "${log_file_names}" | grep -E 'aj/groupmsg/pubconfirm' | grep FS_INFO_APP | awk -F \"|\" 'BEGIN{OFS=\"#_#\"}{ print \$3,\$10}' >> "${all_params_name};
